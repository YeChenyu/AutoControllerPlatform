...
import com.auto.surveyplatform.util.BCDDecode;
import com.auto.surveyplatform.util.LrcUtil;
...
public class HandleProtocol {
	...
	public byte[] packRequestProtocol(byte[] cmd, byte[] params, byte type) {
		...
		System.out.println( "packProtocol: lrcData:"+ StringUtil.byte2HexStr(lrc_data));
...
//		send.append("==============发送 Start===============\n");
		...
		send.append("Send >>> Content:[" + StringUtil.byte2HexStr(params) + "]\n\n");
//		send.append("==============发送 End===============\n");
...
//		recv.append("==============接收 Start===============\n");
		...
		for (String[] cmdPair : ALL_CMD) {
			if (cmdPair[0].equalsIgnoreCase(cmdStr)) {
				recv.append("Receive <<< CMD:[" + cmdStr + " " + cmdPair[1] + "]\n");
				break;
			}
		}
		recv.append("Receive <<< Resp:["+respCode+"] Content:[" + StringUtil.byte2HexStr(params) + "]\n");
//		recv.append("==============接收 End===============\n");
